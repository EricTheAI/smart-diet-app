...
export class TodayPage {
  ...
  private weight:number=65;
  private energy:any=0;
  private protein:any=0;
  private fat:any =0;
  private carbo:any =0;
  private maxenergy:number=1800;
  private maxprotein:number=12*65;
  private maxcarbo:number=5*65;
  private maxfat:number=0.8*65;
  private max={'Protein':0, 'Energy':0, 'Carbonhydrate':0, 'Fat':0};
  ...
  var labelSetting = {
      normal: {
          ...
          formatter: function (param) {
              return (param.value*param.data.label/100).toFixed(1);
          },
          textStyle: {
              fontSize: 18,
              fontFamily: 'Arial'
  ...
  var option = {
      ...
      xAxis: {
          data: ['Energy', 'Protein', 'Carbonhydrate', 'Fat'],
          axisTick: {show: false},
          axisLine: {show: false},
          axisLabel: {show: true}
      },
      ...
      series: [{
          ...
          label: labelSetting,
          data: [{
              value: this.energy/this.maxenergy*100,
              symbol: symbols[0],
              label:this.maxenergy
          }, {
              value: this.protein/this.maxprotein*100,
              symbol: symbols[1],
              label:this.maxprotein
          }, {
              value: this.carbo/this.maxcarbo*100,
              symbol: symbols[2],
              label:this.maxcarbo
          }, {
              value: this.fat/this.maxfat*100,
              symbol: symbols[3],
              label:this.maxfat
          }],
          z: 10
  ...
  initializeItems() {
    ...
    for (var i in fooddata){
      this.items.push(fooddata[i].name);
    }
  }
  ...
  calhealth(){
    ...
    this.carbo=0;
    this.fat=0;
    console.log(this.httpservice.getuserprofile());
    var allfood=this.httpservice.getFoodData();
    var foodlist=this.httpservice.getfoodlist();
    var goal=this.httpservice.getuserprofile()["goal"];
    var gender=this.httpservice.getuserprofile()["gender"];
    var weight=this.httpservice.getuserprofile()["weight"];
    if (!weight){
      weight=65;
    }
    if (goal=="loseweight"){
      this.maxprotein=weight;
      this.maxfat=0.8*weight;
      this.maxcarbo=5*weight;
      if (gender=="Male"){
        this.maxenergy=1900;
      }else{
        this.maxenergy=1700;
      ...
      this.maxprotein=1.2*weight;
      this.maxfat=1.25*weight;
      this.maxcarbo=7.5*weight;
      this.maxenergy=1900;
    }else if(goal=="getmuscle"){
      this.maxprotein=2.5*weight;
      this.maxfat=1.28*weight;
      this.maxcarbo=10.2*weight;
      if (gender=="Male"){
        this.maxenergy=2700;
      }else{
        this.maxenergy=2200;
    ...
    this.max["Protein"]=this.maxprotein;
    this.max["Energy"]=this.maxenergy;
    this.max["Carbonhydrate"]=this.maxcarbo;
    this.max["Fat"]=this.maxfat;
    ...
    for (var i in foodlist){
      ...
      for (var j in allfood){
        ...
        if (allfood[j].name==food){
          this.protein+=allfood[j].protein*Number(quan);
          this.energy+=allfood[j].energy*Number(quan);
          this.carbo+=allfood[j].carbohydrates*Number(quan);
          this.fat+=allfood[j].fat*Number(quan);
        }
